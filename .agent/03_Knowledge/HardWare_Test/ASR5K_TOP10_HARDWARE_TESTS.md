...
| 7 | Incomplete page guard | Run Test5-Test7: select page 1, write four samples, then read status. | State remains `DOWNLOADING`; `RX_DONE` is clear; count is 4; last address is `0x3003`; continuity is true. |
...
| 9 | Full burst pipeline | Run Test9: select, burst 4096 samples, poll status, validate, activate. | Test9 PASS; status reaches `REG_READY | RX_DONE`; state `LOCKED=6`; sample count 4096; DMA delta minimum `>=4107` (poll retries may increase it); parse fail and all faults are 0. |
