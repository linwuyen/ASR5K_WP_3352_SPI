...
| CR-007 | 0x095B / WAVE_BURST_BEGIN Conflict | Older flow designs omit 0x095B and only describe standard registers (`0x0958` / `0x3000..0x3FFF` / `0x0959` / `0x0960` / `0x0961`). | `0x095B` is a normal Legacy `ADDR + DATA` register write, NOT Packet Protocol. It is the official production register used to pre-arm the block-DMA burst path. Senders that omit `0x095B` fall back to compatibility overflow detection, which is best-effort and not guaranteed lossless. Senders must use `0x095B = 4096` and guard/trailing frames for lossless block-DMA. Packet Protocol framing from D02/D11 must not be used. | D02, D02_2_1, D11 | CLOSED - production meaning resolved; historical source cleanup deferred |
