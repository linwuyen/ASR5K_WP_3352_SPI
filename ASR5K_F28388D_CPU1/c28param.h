...
typedef volatile struct
{
    ...
    /* DDS Control Variables (JTAG writable) */
    uint32_t u32TargetFreq_x100; /**< Target frequency * 100 (e.g., 5000 = 50.00Hz) */
    uint16_t u16TargetAmp;       /**< Target amplitude (0-32767) */
    uint16_t u16TargetOff;       /**< Target DC offset (typically 32767) */
    float f32DelayOnTime;        /**< Delay-on time in seconds (0.000-60.000) */
    float f32DelayOffTime;       /**< Delay-off time in seconds (0.000-60.000) */
    float f32PhaseOnAngle;       /**< Phase-on angle (0-360°, -1=immediate) */
    float f32PhaseOffAngle;      /**< Phase-off angle (0-360°, -1=immediate) */
    uint16_t bDdsStartCmd;       /**< Start command flag (set to 1 via JTAG) */
    uint16_t bDdsStopCmd;        /**< Stop command flag (set to 1 via JTAG) */

    /* DDS Ramp Control Parameters (JTAG writable) - Time in seconds (0.000-60.000) */
    float32_t f32AmpRampTimeUp;      /**< Amplitude ramp up time in seconds */
    float32_t f32AmpRampTimeDown;    /**< Amplitude ramp down time in seconds */
    uint16_t bAmpRampEnabled;        /**< Amplitude ramp enable flag */
    float32_t f32OffsetRampTimeUp;   /**< Offset ramp up time in seconds */
    float32_t f32OffsetRampTimeDown; /**< Offset ramp down time in seconds */
    uint16_t bOffsetRampEnabled;     /**< Offset ramp enable flag */
    float32_t f32FreqRampTimeUp;     /**< Frequency ramp up time in seconds */
    float32_t f32FreqRampTimeDown;   /**< Frequency ramp down time in seconds */
    uint16_t bFreqRampEnabled;       /**< Frequency ramp enable flag */

    /* DDS Status Variables (JTAG readable) */
    uint16_t bDdsReady;           /**< DDS ready status */
    uint16_t bDdsRunning;         /**< DDS running status */
    uint16_t u16DdsSample;        /**< Current DDS sample output */
    uint32_t u32DdsUpdateCounter; /**< Update counter for monitoring */

    /* DDS Ramp Status Variables (JTAG readable) */
    uint16_t bAmpRampComplete;    /**< Amplitude ramp complete status */
    uint16_t bOffsetRampComplete; /**< Offset ramp complete status */
    uint16_t bFreqRampComplete;   /**< Frequency ramp complete status */
    uint16_t bAnyRampActive;      /**< Any ramp currently active */
    uint16_t u16CurrentAmp;       /**< Current amplitude value */
    uint16_t u16CurrentOffset;    /**< Current offset value */
    uint32_t u32CurrentFreq_x100; /**< Current frequency value */

    /* DDS Ramp Debug Variables (JTAG readable) */
    uint16_t u16FreqRampStatus;  /**< Frequency ramp status (0=idle, 1=up, 2=down, 3=complete) */
    uint32_t u32FreqRampCurrent; /**< Frequency ramp current value */
    uint32_t u32FreqRampTarget;  /**< Frequency ramp target value */
    uint16_t u16ActiveRampMask;  /**< Active ramp mask for debugging */
