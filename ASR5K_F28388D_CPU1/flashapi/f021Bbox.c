...
#include "f021Bbox.h"
#include <string.h>
#pragma CODE_SECTION(formatFlashSection, ".fapi_ram")
#pragma CODE_SECTION(runFlashStorage, ".fapi_ram")
#pragma CODE_SECTION(initFlashStorage, ".fapi_ram")
#pragma CODE_SECTION(prepareFlashData, ".fapi_ram")
#pragma CODE_SECTION(writeDataToFlash, ".fapi_ram")
#pragma CODE_SECTION(restoreInitialValuesToRAM, ".fapi_ram")
#pragma CODE_SECTION(recalParameters, ".fapi_ram")
...
/** 
 * @brief Compile-time safety check for Flash boundary
 * @note Reference: 2837xD_FLASH_lnk_cpu1.cmd
 *       FLASHN : origin = 0x0BE000, length = 0x001FF0
 * The last Flash Sector (Sector N) is defined as 0x1FF0 length in the linker file,
 * which is 0x10 (16 words) shorter than the full sector size (0x2000).
 * This check ensures that the circular buffer logic (which assumes full 0x400 parts)
 * does not write into this reserved area when using the last partition.
 */
#define RESERVED_FLASH_WORDS 0x10u
#define TOTAL_BBOX_WORDS     (SIZE_OF_BBOX * sizeof(DATA_128BITS))
 
/* 
 * Safety Check Logic:
 * Total BBox Size + Reserved Area must fit within one Partition (PART_SIZE).
 * Using addition avoids negative number underflow issues during comparison.
 */
#define IS_BBOX_SAFE  ((TOTAL_BBOX_WORDS + RESERVED_FLASH_WORDS) <= PART_SIZE)

/* 
 * Static Assertion:
 * We use a negative bit-field width to trigger a hard compilation error.
 * If IS_BBOX_SAFE is false (0), the width becomes -1, which is illegal.
 */
struct FLASH_BOUNDARY_CHECK { 
    unsigned int BBOX_LIMIT_EXCEEDED : (IS_BBOX_SAFE) ? 1 : -1; 
};
...
void initFlashStorage(void)
{
    ...
    case _CHECK_INIT_FLASH_READY:
        ...
        {
            // [FIX] Boot with RAM defaults if Flash read fails to prevent bricking
            sBbox.u32Stat = (_BBOX_FREE | _READ_BACK_FROM_BBOX | _MARK_ERROR_FOR_BBOX);
            sBbox.fsm = _BBOX_FREE;
        }
        break;
...
void writeDataToFlash(void)
{
    ...
    case _ENABLE_FLASH_PUMP_FOR_BBOX:
        Flash_claimPumpSemaphore(FLASH_PUMP_FOR_CPUX);
        sBbox.u32DelayCnt = FAPI_DELAY_CNTS;
    ...
    case _PROGRAM_BBOX_VAR_INTO_FLASH:
        // [FIX] Single-Item Blocking Write
        // We write ONE item, wait for it to complete in RAM (Safety), then return.
        // This avoids RWW conflicts while allowing the main loop to run between writes.
        if (sBbox.u32Index < sBbox.u32Ammount)
        {
            ...
            // Blocking Wait: Ensure CPU stays in RAM until THIS write is done.
    ...
    // case _WAIT_FOR_PROGRAM_BBOX_READY: // Removed: handled by blocking loop above

    case _FORMAT_OLDEST_SECTION:
        // [FIX] Execute Blocking Erase in RAM with DINT protection
        // This prevents returning to Flash while Bank 0 is busy erasing.
        DINT;
        EALLOW;
        // Issue Erase Command
        sBbox.oReturnCheck = Fapi_issueAsyncCommandWithAddress(Fapi_EraseSector, (uint32 *)sBbox.u32OldestSection);
        
        // Wait until Ready (Blocking Loop)
        while(Fapi_checkFsmForReady() == Fapi_Status_FsmBusy)
        {
            // SysCtl_serviceWatchdog(); // Optional
        }
        
        // Restore Interrupts only after Erase is complete
        EDIS;
        EINT;

        if (Fapi_Status_Success == sBbox.oReturnCheck)
        {
            // Erase Done, proceed to write
            sBbox.fsm = _WRITE_BBOX_INTO_FLASH;
        }
        break;
...
{
    // [FIX] Blocking Implementation in RAM to prevent RWW Conflict
    DINT;
    EALLOW;
    ...
    // 2. Init API
    Fapi_initializeAPI(F021_CPU0_BASE_ADDRESS, CPUCLK_FREQUENCY);
    Fapi_setActiveFlashBank(Fapi_FlashBank0);

    // 3. Blocking Erase Loop (Safe in RAM)
    uint32_t u32Address = START_OF_PART_ADDRESS;
    while(u32Address < END_OF_PART_ADDRESS)
    {
        Fapi_issueAsyncCommandWithAddress(Fapi_EraseSector, (uint32 *)u32Address);
        
        while(Fapi_checkFsmForReady() == Fapi_Status_FsmBusy)
        {
            // SysCtl_serviceWatchdog(); // Optional if WD active
        }
        u32Address += SECTION_SIZE;
    }
    ...
    // 5. Transition to Write State (writes will happen in next runFlashStorage loop)
    // Note: writeDataToFlash must also be in RAM (handled by pragma)
    sBbox.fsm = _WRITE_BBOX_INTO_FLASH;
